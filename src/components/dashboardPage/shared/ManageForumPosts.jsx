...
export default function ManageForumPosts({ role = "trainer" }) {
  ...
  const description = isAdmin 
    ...
    : "Manage your contributions to the FitFlow community.";
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link 
          href={newPostUrl}
          className="flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700 dark:bg-blue-500 dark:text-white dark:hover:bg-blue-600 dark:shadow-none transition-all hover:scale-105 active:scale-95"
        >
      ...
      <Card className="flex flex-col gap-4 sm:flex-row sm:items-center justify-between border-slate-200 dark:border-slate-800 bg-card/50 backdrop-blur-sm p-4 shadow-sm rounded-xl">
        <div className="relative w-full flex-1">
          ...
          <Input
            ...
            className="h-11 rounded-xl border-slate-200 dark:border-slate-800 bg-background/50 pl-11 pr-4 text-sm font-medium focus-visible:ring-blue-500/50"
          />
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {isAdmin && (
            <Select value={authorFilter} onValueChange={setAuthorFilter}>
              <SelectTrigger className="h-11 w-40 rounded-xl border-slate-200 dark:border-slate-800 bg-background/50 text-sm font-medium focus:ring-blue-500/50">
        ...
        <Card className="overflow-hidden border-slate-200 dark:border-slate-800 bg-card/50 backdrop-blur-sm shadow-sm rounded-xl">
          <Table>
            ...
            <TableBody>
              {error ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-8 text-blue-500 font-medium">
                  {error}
                </TableCell>
              </TableRow>
            ) : posts.length === 0 ? (
                ...
                <TableRow key={post._id} className="border-slate-200 dark:border-slate-800 group hover:bg-muted/20 even:bg-muted/10 transition-colors">
                  ...
                  <TableCell className="px-6 py-4">
                    <div className="flex items-center gap-4 text-muted-foreground font-medium">
                      ...
                      {isAdmin && <span className="flex items-center gap-1"><ThumbsDown className="size-3.5 text-blue-400" /> {post.downvotes || 0}</span>}
                  ...
                  <TableCell className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button 
                        ...
                        className="inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold transition-all bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-900 dark:hover:bg-blue-900/50"
      ...
      {postToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <Card className="w-full max-w-sm p-6 bg-background rounded-xl shadow-2xl space-y-6 text-center border-slate-200 dark:border-slate-800">
            <div className="mx-auto flex size-12 items-center justify-center rounded-2xl bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 mb-4">
            ...
            <div className="flex justify-center gap-3 pt-4">
              ...
              <button 
                ...
                className="px-5 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:text-white dark:hover:bg-blue-600 rounded-xl shadow-lg shadow-blue-600/20 dark:shadow-none transition-all disabled:opacity-50 hover:scale-105 active:scale-95"
                disabled={isDeleting}
              >
